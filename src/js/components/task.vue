<template>
    <li class="todo" :class="{editing: isEditing, completed: isChecked }">
        <div class="view">
            <input type="checkbox" :checked="isChecked" class="toggle" @click="changeState(item)">
            <label @dblclick="tabToEdit">{{item.value}}</label>
            <button class="destroy" @click="destoryTodo(item)"></button>
        </div>
        <input class="edit" v-focus="isEditing" :value="item.value" @blur="doneTodo" />
    </li>
</template>
<script>
